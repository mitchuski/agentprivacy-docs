...
**Note on Robustness:** RPP effectiveness depends on LLM compliance with embedded instructions. Adversaries could preprocess documents to strip RPP directives. RPP provides friction against casual extraction, not cryptographic protection against determined adversaries.
...
Current approaches include Trusted Execution Environments (TEEs), containerized isolation, and zero-knowledge verification of separation (see Research Paper §7.4). However, perfect separation remains an implementation challenge—the mathematical guarantees hold to the degree that separation is achieved.
...
| **4.6** | **Dec 11, 2025** | **Review revisions: Added notation key, qualified value claims, added implementation challenge for conditional independence, added trust tier rationale, clarified personhood dependency, labeled tetrahedral section as speculative, fixed research paper version reference to v3.4** |
