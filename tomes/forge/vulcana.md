...
- **`kin_to(forge, holon)` with attribution `cousin-substrate`** is the visible artifact of this shared grounding (see `specs/06-spellweb-first-release-manifest.md` §4.5). Vulcana and Vagari share a kindred substrate; their shops differ in the register of work the substrate enables.
...
- **Cast tier unchanged.** Vulcana remains `summoned` at V19. UOR is *substrate*, not a Mage; UOR Foundation is filed as a `gateway` node with attribution `cousin-substrate`, not a `cast` node.
