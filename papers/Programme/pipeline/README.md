...
pipeline/
  ...
  SOURCES.md             corpus registry · resolves every extraction source slug to paths · GR-9 companion
  ...
  chronicles/            per-session path record · one file per session, never edit another's · feeds the methods extraction
  ...
  tools/                 fleet_tracker.py · generates tracker/ (wall-board + rendered reader pages)
  tracker/               GENERATED · never hand-edit · rebuild via tools/fleet_tracker.py
```
...
```
cd pipeline/checks
python3 check_register_refs.py  <files>   # C-number hygiene per tier · head C96
...
Keeping `HEAD` in `check_register_refs.py` synchronised with `manifest.yaml`'s `register_head` is on A9's sweep list (both at C96 since 2026-07-02, L005).

## The fleet tracker (wall-board)

`python tools/fleet_tracker.py --serve` regenerates `tracker/` and serves it at `http://localhost:7272`: the first-person decision queue, the artifact board with gate tracks, the extraction shelf, the session timeline, the ledger tail, the checks matrix, and every pipeline document rendered human-readable (canon linked read-only). A0 reruns the build at every cycle close-out, in the same breath as the checks. GR-6 applies: `tracker/` is generated output — fix the sources or `tools/fleet_tracker.py`, never the HTML. The tracker displays state and never owns it: the manifest stays the map, the ledger stays the memory.
