...
\section{Privacy is Value · V6: The Gathering Turn and the Moving
Ceiling}\label{privacy-is-value-v6-the-gathering-turn-and-the-moving-ceiling}

\subsection{Formal Specification of the Privacy Value Model · Dual-Agent
Privacy
...
publication (2026-06-10), C93 after the Limitative Reading addendum
(Band IX, §17.10, 2026-06-28) · when this document and the register
disagree, the register wins \textbf{Suite labeling (G3 decision):} all
canon papers carry the unified V6 label: this formal specification · the
compressed Swordsman reading · the companion Mage reading · the research
paper · the whitepaper V6 edition. One label, one current state of the
model.
...
\subsection{The Document Suite}\label{the-document-suite}
...
\subsection{1. The Equation}\label{the-equation}

\subsubsection{1.1 Static Form}\label{static-form}
...
\subsubsection{1.2 Differential Form}\label{differential-form}
...
\subsubsection{1.3 Multiplicative Gating}\label{multiplicative-gating}
...
\subsubsection{1.4 Master Inscription}\label{master-inscription}
...
\subsection{2. Inherited Terms (V1--V4)}\label{inherited-terms-v1v4}
...
\subsection{\texorpdfstring{3. Holonic Temporal Memory ·
\(A_h(\tau)\)}{3. Holonic Temporal Memory · A\_h(\textbackslash tau)}}\label{holonic-temporal-memory-a_htau}

\subsubsection{3.1 Motivation}\label{motivation}
...
\subsubsection{3.3 Properties}\label{properties}
...
\subsubsection{3.4 GUID Structure}\label{guid-structure}
...
\subsection{\texorpdfstring{4. Three-Axis Separation ·
...
\subsubsection{4.2 Agent-Layer Separation}\label{agent-layer-separation}
...
\subsubsection{4.3 Data-Layer Separation}\label{data-layer-separation}
...
\subsubsection{4.4 Inference-Layer
...
\subsection{\texorpdfstring{5. Reconstruction Difficulty ·
...
\subsubsection{5.2 Compression-as-Defence}\label{compression-as-defence}
...
\subsubsection{5.3 Behavioural Density ρ
...
\subsubsection{5.4 The Reconstruction Ceiling (Proven, conditional
...
analysis via Fano's inequality, read as a decomposition: the
preconditions of §10.5 yield the additive capacity sum and the error
floor, and the budget constraint is the capacity-deficit condition C\_S
+ C\_M \textless{} H(X) of §11.1, the numerical condition that places
the ceiling strictly below one. See §16 for the full proof status, and
§10 to §11 for the preconditions and the capacity-deficit condition
...
\subsubsection{5.5 The Moving Ceiling R(t)
...
\subsection{6. Network Effects and Guild
Efficiency}\label{network-effects-and-guild-efficiency}

\subsubsection{6.1 Network Term}\label{network-term}
...
\subsubsection{6.2 Guild Efficiency}\label{guild-efficiency}
...
\subsubsection{6.3 Conjecture C10}\label{conjecture-c10}
...
\subsection{\texorpdfstring{7. Path Integral Edge Value ·
...
\subsubsection{7.2 Fidelity Component
...
\subsubsection{7.3 Conjecture C15}\label{conjecture-c15}
...
\subsection{8. The Holographic Bound}\label{the-holographic-bound}

\subsubsection{8.1 Structure}\label{structure}

\[\partial M: \text{96 edges encoding 64 vertices, toroidal topology}\]

\[\frac{96}{64} = 1.5 = P^{1.5} \text{ exponent}\]

\subsubsection{8.2 Resolution of C4}\label{resolution-of-c4}
...
\subsubsection{8.3 Algebraic Confirmation}\label{algebraic-confirmation}
...
\subsubsection{8.4 Implications}\label{implications}
...
\subsubsection{8.5 Conjecture C6}\label{conjecture-c6}
...
\subsection{9. Differential Form}\label{differential-form-1}

\subsubsection{9.1 Specification}\label{specification}
...
\subsubsection{9.2 Five-Channel
...
\subsubsection{9.3 Named Seam (V6)}\label{named-seam-v6}
...
\subsection{10. The Separation Bound}\label{the-separation-bound}

\subsubsection{10.1 Core Guarantee}\label{core-guarantee}
...
\subsubsection{10.2 Betweenness Centrality of the
...
\subsubsection{10.3 Composite Separation}\label{composite-separation}
...
\subsubsection{10.4 Amnesia vs Policy Bounds (V5.3,
...
\subsubsection{10.5 Preconditions (V6)}\label{preconditions-v6}
...
boundary condition: \textbf{the additive capacity sum, and with it the
error floor P\_e ≥ 1 − R\_max, hold in the regime the Amnesia Protocol
is designed to enforce, and only there; the strict ceiling R\_max
\textless{} 1 requires in addition the capacity-deficit condition of
§11.1.}
...
\textbf{The capacity-deficit condition (not a third precondition).} The
two preconditions are architectural: attestable properties of the
deployment. Neither constrains the sum C\_S + C\_M relative to H(X).
What they buy is the additive leakage structure and the error floor P\_e
≥ 1 − R\_max; the strict bound R\_max \textless{} 1 holds exactly when,
additionally, C\_S + C\_M \textless{} H(X). That capacity deficit is a
numerical fact about a given system and adversary class: measurable,
declarable, and eroded over time as stronger classes arrive (§5.5). It
is deliberately not listed as a third precondition, because two
conditionally independent channels of sufficient combined capacity
satisfy both preconditions with R\_max at or above one; the deficit is
the separate, system-specific fact a deployment measures and declares.
§11 states the decomposition as the theorem's conditioning.

\subsubsection{10.6 External Grounding
(V6)}\label{external-grounding-v6}
...
the field already accepts, under named preconditions and a declared
capacity-deficit condition.''

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{11. The Reconstruction
Ceiling}\label{the-reconstruction-ceiling}

\subsubsection{11.1 Theorem (Proven, conditional
...
The theorem decomposes. The two preconditions of §10.5 license the
capacity sum and yield the error floor of §11.2; they do not by
themselves place \(R_{\max}\) below one. The strict inequality is the
\textbf{capacity-deficit condition} \(C_S + C_M < H(X)\): a measurable,
declarable, numerical fact about a given system and adversary class, not
a consequence of the architecture (§10.5).

\textbf{Consequence:} When the two preconditions and the
capacity-deficit condition hold together, perfect reconstruction of the
First Person's state is impossible against the stated adversary class.

\subsubsection{11.2 Error Floor (Proven, conditional
regime)}\label{error-floor-proven-conditional-regime}

\[P_e \geq 1 - R_{\max}\]

The adversary is guaranteed to make errors whenever the capacity-deficit
condition of §11.1 holds. The floor itself follows from Fano's
inequality and holds under the two preconditions of §10.5; at
\(R_{\max} \geq 1\) it is vacuous, which is exactly why the deficit
condition must be declared alongside the preconditions.

\subsubsection{11.3 Graceful Degradation (Proven, conditional
...
\subsubsection{11.4 Dynamical Reconstruction Ceiling (V6 Horizon,
...
\subsubsection{11.5 Conditioning (V6)}\label{conditioning-v6}

R\_max = (C\_S + C\_M)/H(X) \textless{} 1 carries the label
\textbf{Proven, conditional regime}, read as a decomposition: within
Preconditions 1 and 2 of §10 the capacity sum is licensed and the error
floor P\_e ≥ 1 − R\_max (Fano converse) is proven, an instance of the
family cited there; the strict bound R\_max \textless{} 1 holds when,
additionally, the capacity-deficit condition C\_S + C\_M \textless{}
H(X) of §11.1 holds. The deficit condition, not the preconditions, is
the time-indexed quantity per §5.5: R(t) can cross one with both
preconditions intact, because capability growth raises the effective
capacities, and what expires at t* is the deficit condition, not the
architecture. Outside the regime, §26 governs.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{12. Algebraic Foundation ·
Z/(2⁶)Z}\label{algebraic-foundation-z2ux2076z}

\subsubsection{12.1 Ring Structure}\label{ring-structure}
...
\subsubsection{12.2 The Five Hammer
Strikes}\label{the-five-hammer-strikes}
...
\subsubsection{12.3 Critical Identity}\label{critical-identity}
...
\subsubsection{12.4 Triadic Coordinates
...
\subsubsection{12.5 Dihedral Group
D₆₄}\label{dihedral-group-dux2086ux2084}
...
\subsubsection{12.6 Six Sovereignty
...
\textbf{Numeric encoding (ruled 2026-06-12 · the MODEL lock).} The
string \([d_1 d_2 d_3 d_4 d_5 d_6]\) reads most-significant-bit first:
\(d_k\) carries weight \(2^{6-k}\), so \(d_1\) Protection is the high
bit (32) and \(d_6\) Value the low bit (1). Worked anchors: V38 =
\texttt{100110} = Protection+Connection+Computation (Aletheia, the
bright medium); V25 = \texttt{011001} = Delegation+Memory+Value (Lethe,
the dark substrate); V41 = \texttt{101001} = Protection+Memory+Value
(Memora). This is consistent with the Aletheia/Lethe reading in §12.8
and with every pinned grimoire seat. Any surface reading \(d_1\) at the
low bit is in erratum to this ruling.

\subsubsection{12.7 External Convergence}\label{external-convergence}
...
\subsubsection{12.8 The ARCH-1 Bridge (C85)
...
\subsubsection{12.9 The Operational Layer: ARCH-1R/T Seated (C72 to C76)
...
\subsubsection{13.1 Definition}\label{definition-4}
...
\subsubsection{13.2 Relationship to Path
...
\subsection{14. The Amnesia Protocol
(V5.3)}\label{the-amnesia-protocol-v5.3}

\subsubsection{14.1 Definition}\label{definition-5}
...
\subsubsection{14.2 Zero-Knowledge
...
\subsubsection{14.3 Implementation
...
\subsubsection{14.4 Conjecture C17}\label{conjecture-c17}
...
\subsubsection{14.5 Selene's Proof}\label{selenes-proof}
...
\subsubsection{14.6 The Obstruction Upgrade (C86)
...
\subsubsection{14.7 Separation Made Quantitative (C83)
...
\subsection{15. Ceremony and Forge
Implementation}\label{ceremony-and-forge-implementation}

\subsubsection{15.1 Operational Cycle as
...
\subsubsection{15.2 Progressive Trust}\label{progressive-trust}
...
\subsubsection{15.3 Forge Cryptographic
...
\subsubsection{15.4 Runecraft as Φ\_agent
...
\subsubsection{15.5 Moon Phase as Visibility
...
\subsubsection{15.6 Tier Classification
...
\subsubsection{15.7 The Trust Recursion as Folding Scheme (C87)
...
\subsubsection{15.8 The Presence Economy Regime
...
\subsubsection{15.9 The Key as a Reading (C66)
...
\subsection{16. Proven Results}\label{proven-results}
...
under budget constraints, read per §11: the §10.5 preconditions license
the sum and yield the error floor; the budget constraint is the
capacity-deficit condition \(C_S + C_M < H(X)\) that places the ceiling
below one \\
...
\subsection{17. The Conjecture Register}\label{the-conjecture-register}
...
\subsubsection{17.1 How to Read the
Tables}\label{how-to-read-the-tables}
...
\subsubsection{17.2 Band I · V1 to V5.3 Core (C1 to
...
\subsubsection{17.3 Band II · V6 Research Series, April 2026 (C18 to
...
\subsubsection{17.4 Band III · Bound Collection, Tomes IV to V (C38 to
...
\subsubsection{17.5 Band IV · Post-V5.4 Coherence Set (C47 to
...
\subsubsection{17.6 Band V · City Register Continuation (C56 to
...
\subsubsection{17.7 Band VI · Horizon District (C67 to C71) · PINNED in
Grimoire
...
\subsubsection{17.8 Band VII · Renumbered Research Blocks (C72 to C81) ·
Assigned at Run
...
\subsubsection{17.9 Band VIII · Registered During the V6 Runs
...
\subsubsection{17.10 Band IX · The Limitative Reading (C90 to C93) ·
Addendum
2026-06-28}\label{band-ix-the-limitative-reading-c90-to-c93-addendum-2026-06-28}

\emph{Post-publication addendum. This volume locked at head C89 on
2026-06-10; the Limitative Reading was registered at Gate G6 on
2026-06-28 (Run 8 of the autopath) and is reproduced here for
completeness. Home note:
\texttt{research/limitative-theorems-and-privacy-is-value.md}. These are
framing conjectures: the V6 ceilings and the existence-leak law read as
privacy-flavoured instances of Gödel and Tarski. Every join is
structural framing (\textasciitilde80\%), not a theorem-to-theorem
reduction (\textasciitilde50\%); the arithmetisation correspondence
(Gödel numbering ↔ Z/(2⁶)Z) is intuition, not load-bearing.}
...
\bottomrule\noalign{}
\endlastfoot
C90 & The Limitative Inversion: completeness ⇒ Φ → 0 ⇒ collapse is the
value-sign reversal of completeness ⇒ inconsistency ⇒ collapse; the
unreconstructable remainder is load-bearing. C17 in limitative terms &
\textasciitilde90\% as observation & observation · registered Run 8,
2026-06-28 · no reduction claimed & core & limitative-theorems note §1,
§4 \\
C91 & Gödel ↔ Φ\_agent: zero-memory (Selene) is the Φ\_agent instance of
Gödel's first theorem; a witness real yet underivable from within,
intrinsic to a single system; destroying it is structural separation &
\textasciitilde60\% & active · registered Run 8, 2026-06-28 & core &
limitative-theorems note §2, §3.3 \\
C92 & Tarski ↔ Φ\_inference: existence-leak is the Tarski-undefinability
instance loading on Φ\_inference; feasibility-truth escapes containment
across systems; D(X) monotone non-increasing in corroborating systems;
absorbs the Gödelian seed (existence as positive provability); rides on
C81, cannot exceed its base & \textasciitilde70\% & active · registered
Run 8, 2026-06-28 & core & limitative-theorems note §3 \\
C93 & Content-addressed liveness leak: a live content-address is an
existence claim about its content; deduplication/GUID liveness leaks
existence, and existence bounds the search; the address does not leak
content, its liveness leaks existence & \textasciitilde55\% & active ·
registered Run 8, 2026-06-28 & core & limitative-theorems note §3.5 \\
\end{longtable}
}

\emph{Open seam (no number): does Φ\_data have a limitative twin, or
does it fail by degree (Φ\_data = 1 − 1/\textbar providers\textbar)
rather than undecidability? Left open at G6. Decides whether Φ\_v5
stands on two limitative theorems plus a degree-of-freedom or on three.}

\subsubsection{17.11 Gate G1 Dispositions (Signed
...
\subsection{18. Measurement Gaps and Breaking
Conditions}\label{measurement-gaps-and-breaking-conditions}

\subsubsection{18.1 Measurement Gaps}\label{measurement-gaps}
...
\subsubsection{18.2 Breaking Conditions}\label{breaking-conditions}
...
\subsubsection{18.3 V6 Breaking
Conditions}\label{v6-breaking-conditions}
...
\subsection{19. Three Identity Layers}\label{three-identity-layers}
...
\subsection{20. Cosmological Quaternion (Interpretive
...
\subsection{21. Compression Spectrum}\label{compression-spectrum}
...
\subsection{22. Promise Theory
Grounding}\label{promise-theory-grounding}
...
\subsubsection{22.1 Autonomy Axiom}\label{autonomy-axiom}
...
\subsubsection{22.2 Key Mappings}\label{key-mappings}
...
\subsubsection{22.3 Formal Reference}\label{formal-reference}
...
\subsubsection{22.4 V6 Edge}\label{v6-edge}
...
\subsection{23. Version Lineage}\label{version-lineage}
...
\subsection{24. Notation Summary}\label{notation-summary}
...
\subsection{25. The Two Instances of
2026}\label{the-two-instances-of-2026}
...
\subsubsection{25.1 Instance 1: Zcash
...
undetectable counterfeit ZEC in a regtest environment. The market
repriced sharply in the days around disclosure, an effect confounded by
a concurrent, prominent institutional exit and therefore not
attributable to the flaw alone; the issue was fixed by the NU6.2 hard
fork at block 3,364,600 on 2026-06-03. Reading in R(t) terms: the
circuit's H(X) never changed; the decoder improved overnight; four
findable years collapsed into one found day.

\subsubsection{25.2 Instance 2: The Schrottenloher
...
\subsubsection{25.3 The Existence-Leak Law (C81, Promoted to
...
\subsubsection{25.4 What the Law Is Not}\label{what-the-law-is-not}
...
\subsubsection{25.5 Stage 2, Stated}\label{stage-2-stated}
...
\subsection{27. The Temporal Thread}\label{the-temporal-thread}
...
\subsubsection{27.1 The Mosca Coupling: Conjecture
...
\subsection{28. The Geometry of the Gap}\label{the-geometry-of-the-gap}
...
\subsection{29. Narrative Corpus and Convergence
...
\subsection{30. Canonical Figures}\label{canonical-figures}
...
\subsection{31. External Landscape and Standards
...
\subsection{32. Honest Limits}\label{honest-limits}

\subsubsection{32.1 The Moving Ceiling (§5.5,
...
\subsubsection{32.2 The Compounding Absorption (§14.7,
...
\subsubsection{32.3 The Existence Leak (§25,
...
\subsubsection{32.4 The Bridge and the Forgetting (§12.8, §12.9,
...
\subsubsection{32.5 The Key, the Knot, and the Star (§15.7 to §15.9,
...
\subsection{33. References}\label{references}
...
\item
  privacymage (2026). ``Promise Theory Reference v1.5.'' April 20, 2026.
  \emph{agentprivacy-docs/research.} {[}V5.4 integration + historical
  lineage (Burgess \& Fagernes 2007, Burgess 2015){]}
\item
...
\subsubsection{The V6 Research-Note Series and the Register
(agentprivacy-docs/research/)}\label{the-v6-research-note-series-and-the-register-agentprivacy-docsresearch}

The notes this specification grew from, in order of arrival, with
provenance as stated at each home document. Conjecture numbering across
all of them resolves through the register.

\begin{itemize}
\tightlist
\item
  privacymage, with Claude Fable 5 (2026). ``The Conjecture Register.''
  Opened 2026-06-10 (V6 autopath, Run 0); Gate G1 signed 2026-06-10;
  register head C89.
  \emph{agentprivacy-docs/research/CONJECTURE\_REGISTER\_V6.md.}
  {[}AUTHORITATIVE for conjecture numbering across the agentprivacy
  suite; when prose and register disagree, the register wins and the
  prose gets an erratum{]}
\item
  privacymage (2026). ``PVM V6 Horizon Note: From Territory to
  Trajectory --- The Second Person Opens.'' April 12, 2026 (post-V5.4
  lock-in note added May 9). {[}Bridge document; the horizon crossed;
  the C18--C46 lineage into the bound collection{]}
\item
  privacymage (2026). ``PVM V6 Research Note: The Single Sufficient
  Operator --- Three Ceilings.'' April 13, 2026. {[}EML; C22--C25{]}
\item
  privacymage (2026). ``The Dragon's Scales --- A Note on Microsoft's
  Agent Governance Toolkit.'' April 14, 2026. {[}Comparative analysis:
  governance tooling read against the dual-agent architecture{]}
\item
  privacymage / Soulbae, with Claude (ORCID 0009-0001-6557-9135), and
  John Haines / Xarvus, OLMA (ORCID 0009-0001-5809-4690) (2026). ``PVM
  V6 Research Note: ARCH-1 --- The Canonical Form.'' April 14, 2026.
  {[}\textbf{Co-derived in conversation --- external convergence lock.}
  The canonical form Σ := μS.(β ∨ Ω(S,S)) with activation engine ρ,
  locked across the Boolean, continuous, and sovereignty domains;
  C26--C29; the seam later promoted to the ARCH-1 bridge, C85 (§12.8).
  Depends on Sheffer (1913) and Odrzywołek (2026){]}
\item
  privacymage (2026). ``PVM V6 Research Note: The Bakhta Half-Life of
  Trust.'' April 17, 2026 (locked-in May 9). {[}C30--C33{]}
\item
  privacymage (2026). ``PVM V6 Research Note: The Wound and the Cap ---
  Convergence at the Bijective Boundary.'' April 18, 2026 (locked-in May
  9). {[}C34--C37{]}
\item
  privacymage (2026). ``Second Person Spellbook: Act Seeds from
  ARCH-1.'' April 20, 2026. {[}Seed document; repurposed as Tome I--III
  seed material after the bound collection shipped{]}
\item
  privacymage (2026). ``PVM V6.1 Research Note: The Fourth Aging
  Category.'' April 21, 2026. {[}C47--C50; responds to Bakhta (2026),
  \emph{The Half-Life of Trust}{]}
\item
  Travers, M. (privacymage) (2026). ``Convergence Note: Compiled AI and
  the agentprivacy Architecture.'' May 20, 2026. {[}Filed as a path
  taken, not a load-bearing result{]}
\item
  privacymage (2026). ``V6 Research Note: The Existence-Leak and the
  Falling Z.'' June 2, 2026. {[}The worked empirical instance of C81/C82
  against Schrottenloher (2026); §25{]}
\item
  Haines, J. (Xarvus / Chaos Rider, OLMA · ORCID 0009-0001-5809-4690)
  (2026). \emph{ARCH-1R/T Operational Reachability Framework.} Draft
  Review v2.0, June 2026. {[}\textbf{The source manuscript.} Appends
  operational layers downstream of G without modifying β, Ω, or μ; the
  latent/obstructed distinction (0 ≠ −). Archived:
  \emph{research/ARCH-1RT Operational Reachability Framework v2
  Expanded.pdf}; the v3 notation supplement is archived alongside{]}
\item
  privacymage / Soulbae, with Claude (ORCID 0009-0001-6557-9135);
  contributor John Haines / Xarvus, OLMA (ORCID 0009-0001-5809-4690)
  (2026). ``PVM V6 Research Note: ARCH-1R/T --- The Operational
  Reachability Framework.'' June 4, 2026. {[}Conversion of Haines' Draft
  Review v2.0 into the series, with the dual-ρ collision review; seated
  as C72--C76 (§12.9; the provisional C67--C71 numbering was renumbered
  at Run 0). The review chronicle and the reply letter of the same date
  are archived alongside{]}
\item
  privacymage / Soulbae, with Claude (2026). ``PVM V6 Research Note:
  Convergence on the Integrity Gap --- Bakhta's \emph{Safety by Design}
  read against the Dual Model.'' June 4, 2026. {[}C77--C80; §22.4{]}
\item
  privacymage (2026). ``Research Note: Cryptographic Durability and the
  Quantum Horizon.'' June 9, 2026. {[}The Horizon District grounding;
  C67--C71 as pinned in grimoire v1.8.0{]}
\end{itemize}

\subsubsection{The First Person Spellbook (Grimoire v10.1.0 · 31 Acts ·
...
\subsubsection{Blog Series: Privacy is Value V5
...
\subsubsection{External References · Information
...
\subsubsection{External References · Cryptography and
...
\subsubsection{External References · Graph
...
\subsubsection{External References · Related
...
\subsubsection{External References · Scientific and
...
\subsubsection{External References · Standards and
...
\subsubsection{Live Implementations}\label{live-implementations}
...
\subsubsection{V6 Additions}\label{v6-additions}
...
\item
  ...
  \emph{Transactions of the American Mathematical Society,} 14(4),
  481--488. {[}The Sheffer stroke --- the single sufficient operator
  precedent; dependency of the ARCH-1 canonical form, C26 to C29{]}
\item
  Kothapalli, A., Setty, S., \& Tzialla, I. (2022). ``Nova: Recursive
  Zero-Knowledge Arguments from Folding Schemes.'' \emph{CRYPTO 2022.}
