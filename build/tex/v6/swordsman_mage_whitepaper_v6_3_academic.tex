...
V6.0) and \texttt{research/CONJECTURE\_REGISTER\_V6.md} (head C96); the
...
\textbf{Reconstruction Ceiling (R(t) \textless{} 1)}: Proven-conditional
bound: while the two agents do not collude (the non-collusion
precondition) and the stated adversary class's combined channel
capacities fall short of your private state's entropy (C\_S + C\_M
\textless{} H(X)), that adversary cannot fully reconstruct your private
state from observations. The ceiling is time-indexed, R(t), and moves as
...
\begin{itemize}
\item
  Technical: S ⊥⊥ M \textbar{} X, the time-indexed reconstruction
  ceiling R(t) \textless{} 1 (conditional on non-collusion and a stated
  adversary class), information-theoretic bounds
\item
...
\tightlist
\item
  High compression (at the spellbook corpus's 70:1 compression ratio or
  better) = strong positive assessment
\item
...
This coordination efficiency comes from promise bundle reuse. Once a VRC
is established, the bundle doesn't need re-verification for each
...
\item
  \textbf{Compression efficiency:} Instead of exchanging 500 tokens
...
privacy emerges from a mathematical bound --- holding for as long as the
capacity-deficit condition C\_S + C\_M \textless{} H(X) holds against
the stated adversary class --- rather than from corporate promises.
...
condition --- any zero collapses total value. The 31,000×
accessible-volume value gap under full behavioural capture is now
understood as \textbf{boundary expressiveness}: sovereignty
architectures have expressive boundaries; surveillance has constrained
boundaries. The 96-edge torus surface encodes the 64-vertex bulk --- the
holographic bound.
...
\section{The Reconstruction Ceiling R(t): Information-Theoretic
Privacy}\label{the-reconstruction-ceiling-rt-information-theoretic-privacy}
...
\textbf{Result:} \textbf{while the two agents do not collude and the
stated adversary's combined channel capacity stays below the entropy of
the whole (C\_S + C\_M \textless{} H(X)), the remaining 60 pieces stay
beyond that adversary's reach} for as long as that capacity deficit
holds.

Not hidden. Not encrypted. \textbf{Nonexistent in that adversary's
information space.}

\textbf{Promise Theory:} Those 60 pieces are the \emph{irreducible
promise}---the property that emerges from separation but cannot be
attributed to either agent.
...
perfection. Under its stated preconditions it depends on counting, not
on adversary effort. The ceiling

\textbf{Reconstruction Ceiling (time-indexed, R(t) per PVM V6 §5 and
§11):}

\begin{quote}
\textbf{R\_max = (C\_S + C\_M) / H(X)}
\end{quote}

bounds what any adversary of the stated class can recover. The
preconditions---non-collusion between the agents, capacities evaluated
against a stated adversary class---give the additive structure and the
error floor P\_e ≥ 1 − R\_max. The strict bound R\_max \textless{} 1
holds exactly when the measurable capacity-deficit condition C\_S + C\_M
\textless{} H(X) additionally holds; the preconditions alone do not
place R\_max below one. The capacities are time-indexed: R(t) = (C\_S(t)
+ C\_M(t)) / H(X) rises as the adversary class strengthens, and the
guarantee stands up to the validity horizon t* at which the deficit
condition would expire.

Sovereignty lives in that gap, for as long as the deficit condition
holds.
...
\subsubsection{1. Efficiency Through the Spellbook Corpus's 70:1
Compression
Ratio}\label{efficiency-through-the-spellbook-corpuss-701-compression-ratio}
...
\subsection{The Spellbook Compression Efficiency
Gain}\label{the-spellbook-compression-efficiency-gain}

With spellbook compression (the spellbook corpus's 70:1 compression
ratio), agent coordination moves from full chronicle exchange to spell
transmission with expand-on-demand. Trust signal becomes measurable
through compression fidelity.
...
(\texttt{papers/v6/dualprivacy\_researchpaper\_v6.md}, successor to the
``Dual Privacy Architecture'' line, layered over the v4.3 proof body)
provides mathematical foundations developing from peer-reviewed
information systems and cryptography literature. Rigorous separation
bounds, time-indexed reconstruction ceilings R(t) stated with their
preconditions (non-collusion, a stated adversary class), error floors
grounded in established information theory. Includes Claims
Classification Table distinguishing proven results, semantic frameworks,
...
\item
  Budget constraints establish reconstruction ceilings R(t),
  time-indexed and conditional on non-collusion and the stated adversary
  class
\item
...
\item
  Compression-based VRCs reduce coordination costs in proportion to the
  spellbook corpus's 70:1 compression ratio
\item
...
\begin{itemize}
\item
  First Persons: the primary value-capture tier; returns denominated in
  protocol units and scaling with active participation
\item
  Guardians: validation compensation scaled by stake tier (Dragon tier
  highest)
\item
  Ecosystem operators: revenue scales with guild size (successful 1k-10k
  member guilds)
\item
...
\item
  \textbf{Promise Theory Reference:} v1.5 (companion document --- V5
  integration)
\item
  \textbf{Research Paper:} \emph{Privacy is Value · V6: The Research
  Paper Edition} ---
  \texttt{papers/v6/dualprivacy\_researchpaper\_v6.md} (companion
  document --- layered over the v4.3 proof body)
\item
...
\item
  ...
  \texttt{specs/vrc\_promise\_protocol\_v3\_3.md}, filename retained at
  v3\_3 for link stability)
\item
  \textbf{Glossary:} Glossary Master v4.0 (canonical terminology; V6
  addendum)
\item
...
6.2 & 2026 (date not separately recorded) & V6-era body revision
preceding 6.3; carried in Document Metadata until the 2026-07-02
reconciliation. Change record not retained; row backfilled 2026-07-02
(ledger L015). \\
\textbf{6.3} & \textbf{April 7, 2026} & \textbf{V10.0.0 Grimoire
alignment} (header version). Body version of the present document; row
backfilled 2026-07-02 (ledger L015). \\
6.3 (V6 edition) & June 10, 2026 & \textbf{V6 edition note added} under
the unified-V6 labeling decision (Gate G3): designated \emph{Privacy is
Value · V6: The Whitepaper (Swordsman and Mage)}; model authority PVM
V6.0 + \texttt{research/CONJECTURE\_REGISTER\_V6.md}; static-ceiling
citations read as Proven-conditional with R(t). Body version and date
retained (the April 7, 2026 date is the body's; June 10, 2026 is the
edition's). Document Metadata reconciled 6.2 → 6.3 on 2026-07-02 per
\texttt{reference/PAPERS\_INDEX.md} (ledger L015, L016). \\
\end{longtable}
}
